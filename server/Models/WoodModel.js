...
async function Get()
{
  try{
    ...
    let Records=await pool.request().query("SELECT * from WoodProduct")
...
async function GetById(ID)
{
  try{
    ...
    .query("SELECT * from WoodProduct where ID_Wood = @input_parameter;");
...
// create new Wood
async function Create(WoodData)
{
  try{
    let pool=await config;
    let insertrecord=await pool.request()
    .input('Name', sql.NVARCHAR(50),WoodData.Name)
    .execute('sp_Insert_Wood')
...
async function Update(WoodData){
    try{
        let pool=await config;
        let updaterecord=await pool.request()
        .input('ID',sql.VARCHAR(10),WoodData.ID)
        .input('Name', sql.NVARCHAR(50),WoodData.Name)
        .execute('sp_Update_Wood')
